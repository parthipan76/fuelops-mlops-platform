...
def load_model():
    ...
    if mlflow_uri:
        ...
        logger.info("ℹ  No MLFLOW_MODEL_URI set  using mock model for local dev")
...
# --- Mock prediction (used when no MLflow model available) ---
...
def predict(request: PredictRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        return PredictResponse(
            ...
            confidence_interval={"lower": round(predicted_price - margin, 4), "upper": round(predicted_price + margin, 4)},
            model_version=model_version,
